...
def act_quantization(b):
    ...
    class _uq(torch.autograd.Function):
        ...
        def backward(ctx, grad_output):
            ...
            input, input_q = ctx.saved_tensors
            i = (input > 1.).float()
            # THIS DOES NOT ZERO THE GRADIENT so I'm getting rid of it:
            # grad_alpha = (grad_output * (i + (input_q - input) * (1 - i))).sum()
            # grad_alpha = (grad_output * (i + (0.0)*(1-i))).sum()
            # grad_input = grad_input*(1-i)

            # previously, when input is above the grad range, grad_alpha was equal to grad_output,
            # now is it euql to output * quant difference
            grad_alpha = (grad_output * ((input_q - input)) ).sum()
            
            condition = (input >= 0.0) & (input <= 1.0)# condition = (input >= 0) & (input <= 1) # stupid pytorch does not allow for if else based assignment for some reason
            # zero the gradient where the condititon is not met
            grad_input = torch.where(condition, grad_output, torch.zeros_like(grad_output))
            
            return grad_input, grad_alpha

    return _uq().apply

# activation and weight quantiztion bits are hardcoded here: - TODO: make them a param
class QuantConv2d_project_part2(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False):
        super(QuantConv2d_project_part2, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        
        self.layer_type = 'QuantConv2d_project_part2'
        self.bit = 4
        self.act_q_bit = 2
        self.weight_quant = weight_quantize_fn(w_bit=self.bit)
        self.act_alq = act_quantization(self.act_q_bit)
        # self.act_alq = act_quantization(self.bit)
        # self.act_alpha = torch.nn.Parameter(torch.tensor(8.0))
        self.act_alpha = torch.nn.Parameter(torch.tensor(7.0))
